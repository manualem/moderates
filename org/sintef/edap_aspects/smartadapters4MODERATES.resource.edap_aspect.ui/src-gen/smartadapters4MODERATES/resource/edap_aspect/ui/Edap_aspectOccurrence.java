 ...
 */
public class Edap_aspectOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "smartadapters4MODERATES.resource.edap_aspect.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "smartadapters4MODERATES.resource.edap_aspect.ui.occurences.declaration";
	...
	private smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectTokenScanner tokenScanner;
	 ...
	 */
	public Edap_aspectOccurrence(smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new smartadapters4MODERATES.resource.edap_aspect.mopp.Edap_aspectMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(smartadapters4MODERATES.resource.edap_aspect.ui.Edap_aspectOccurrence.DECLARATION_ANNOTATION_ID);
	}
