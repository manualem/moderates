 ...
 * contains
 * <code>smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectTextDiagnostic</c
 * ode>s it uses the more precise information of this extended diagnostic type.
 */
public class Edap_aspectMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = smartadapters4MODERATES.resource.edap_aspect.mopp.Edap_aspectPlugin.PLUGIN_ID + ".problem";
	/**
	 ...
	 */
	public static void mark(org.eclipse.emf.ecore.resource.Resource resource, final smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectTextDiagnostic diagnostic) {
	...
	private static void createMarkerFromDiagnostic(org.eclipse.core.resources.IFile file, final smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectTextDiagnostic diagnostic) {
		try {
			...
			if (diagnostic.getProblem().getType() == smartadapters4MODERATES.resource.edap_aspect.Edap_aspectEProblemType.ERROR) {
			...
			smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectTextDiagnostic textDiagnostic = (smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectTextDiagnostic) diagnostic;
			...
			java.util.Collection<smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
			...
			if (!sourceIDs.isEmpty()) {
				marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectStringUtil.explode(sourceIDs, "|"));
			}
		} catch (org.eclipse.core.runtime.CoreException ce) {
			if (ce.getMessage().matches("Marker.*not found.")) {
				// ignore
			} else {
				smartadapters4MODERATES.resource.edap_aspect.mopp.Edap_aspectPlugin.logError("Error marking resource:", ce);
			}
		}
	}
