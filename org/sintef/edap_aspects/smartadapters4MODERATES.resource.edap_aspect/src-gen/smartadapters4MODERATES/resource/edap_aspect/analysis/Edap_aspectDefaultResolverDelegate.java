...
public class Edap_aspectDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	
	private static class ReferenceCache implements smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectReferenceCache, org.eclipse.emf.common.notify.Adapter {
	 ...
	 */
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectReferenceResolveResult<ReferenceType> result) {
		try {
			org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
			org.eclipse.emf.ecore.EObject root = smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectEObjectUtil.findRootContainer(container);
	...
	private boolean checkElement(org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectReferenceResolveResult<ReferenceType> result) {
		if (element.eIsProxy()) {
			return true;
		}
	...
	protected String produceDeResolveErrorMessage(org.eclipse.emf.ecore.EObject refObject, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectTextResource resource) {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation o : element.eClass().getEAllOperations()) {
				if (o.getName().toLowerCase().endsWith(NAME_FEATURE) && o.getEParameters().size() == 0 ) {
					String result = (String) smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectEObjectUtil.invokeOperation(element, o);
	...
	private String getName(ReferenceType element) {
		...
		if(element.eIsProxy()) {
			...
			if (fragment != null && fragment.startsWith(smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectContextDependentURIFragment.INTERNAL_URI_FRAGMENT_PREFIX)) {
				fragment = fragment.substring(smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectContextDependentURIFragment.INTERNAL_URI_FRAGMENT_PREFIX.length());
	...
	protected smartadapters4MODERATES.resource.edap_aspect.IEdap_aspectReferenceCache getCache(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectEObjectUtil.findRootContainer(object);
