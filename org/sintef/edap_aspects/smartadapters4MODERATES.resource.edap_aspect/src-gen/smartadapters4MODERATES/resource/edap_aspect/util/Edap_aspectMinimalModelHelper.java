 ...
 */
public class Edap_aspectMinimalModelHelper {
	
	private final static smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectEClassUtil eClassUtil = new smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if(attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = smartadapters4MODERATES.resource.edap_aspect.util.Edap_aspectListUtil.castListUnchecked(value);
