 ...
 ******************************************************************************/
package edap.resource.edap.analysis.helper;
...
public class EDAPHelpers {
	...
	public static Component getExpressionComponentType(Expression exp) {
		if (exp instanceof ComponentReference) {
			ComponentReference cr = (ComponentReference)exp;
			if (cr.getPropertyNavigations().isEmpty()) return cr.getComponent();
			else {
				Type t = cr.getPropertyNavigations().get(cr.getPropertyNavigations().size()-1).getProperty().getType();
