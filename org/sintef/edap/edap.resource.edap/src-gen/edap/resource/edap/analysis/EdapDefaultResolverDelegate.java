...
public class EdapDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, edap.resource.edap.IEdapReferenceResolveResult<ReferenceType> result) {
		try {
			...
			org.eclipse.emf.ecore.EObject root = edap.resource.edap.util.EdapEObjectUtil.findRootContainer(container);
	...
	private boolean checkElement(org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, edap.resource.edap.IEdapReferenceResolveResult<ReferenceType> result) {
		if (element.eIsProxy()) {
			return true;
		}
	...
	private String getName(ReferenceType element) {
		...
		if(element.eIsProxy()) {
			...
			if (fragment != null && fragment.startsWith(edap.resource.edap.IEdapContextDependentURIFragment.INTERNAL_URI_FRAGMENT_PREFIX)) {
				fragment = fragment.substring(edap.resource.edap.IEdapContextDependentURIFragment.INTERNAL_URI_FRAGMENT_PREFIX.length());
