...
public class EdapResource extends org.eclipse.emf.ecore.resource.impl.ResourceImpl implements edap.resource.edap.IEdapTextResource {
	...
	public <ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> void registerContextDependentProxy(edap.resource.edap.IEdapContextDependentURIFragmentFactory<ContainerType, ReferenceType> factory, ContainerType container, org.eclipse.emf.ecore.EReference reference, String id, org.eclipse.emf.ecore.EObject proxyElement, int position) {
		...
		edap.resource.edap.IEdapContextDependentURIFragment<?> uriFragment = factory.create(id, container, reference, position, proxy);
			...
			if (uri != null) {
				...
				if (result == null || result.eIsProxy()) {
					...
					if (errorMessage == null) {
						...
						addProblem(new edap.resource.edap.mopp.EdapProblem(errorMessage, edap.resource.edap.EdapEProblemType.UNRESOLVED_REFERENCE, edap.resource.edap.EdapEProblemSeverity.ERROR), proxy);
					}
				}
				return result;
			}
		...
		for (org.eclipse.emf.ecore.resource.Resource.Diagnostic errorCand : new org.eclipse.emf.common.util.BasicEList<org.eclipse.emf.ecore.resource.Resource.Diagnostic>(diagnostics)) {
			if (errorCand instanceof edap.resource.edap.IEdapTextDiagnostic) {
				if (((edap.resource.edap.IEdapTextDiagnostic) errorCand).wasCausedBy(cause)) {
					...
					edap.resource.edap.mopp.EdapMarkerHelper.unmark(this, cause);
				}
			}
		}
	}
	
	private void attachResolveError(edap.resource.edap.IEdapReferenceResolveResult<?> result, org.eclipse.emf.ecore.EObject proxy) {
		...
		if (errorMessage == null) {
			...
			addProblem(new edap.resource.edap.mopp.EdapProblem(errorMessage, edap.resource.edap.EdapEProblemType.UNRESOLVED_REFERENCE, edap.resource.edap.EdapEProblemSeverity.ERROR, result.getQuickFixes()), proxy);
		}
	}
	
	private void attachResolveWarnings(edap.resource.edap.IEdapReferenceResolveResult<? extends org.eclipse.emf.ecore.EObject> result, org.eclipse.emf.ecore.EObject proxy) {
		assert result != null;
		...
		if (result.wasResolved()) {
			for (edap.resource.edap.IEdapReferenceMapping<? extends org.eclipse.emf.ecore.EObject> mapping : result.getMappings()) {
				...
				addProblem(new edap.resource.edap.mopp.EdapProblem(warningMessage, edap.resource.edap.EdapEProblemType.UNRESOLVED_REFERENCE, edap.resource.edap.EdapEProblemSeverity.WARNING), proxy);
			}
		}
	}
	...
	protected void runPostProcessors(java.util.Map<?, ?> loadOptions) {
		...
		edap.resource.edap.mopp.EdapMarkerHelper.unmark(this, edap.resource.edap.EdapEProblemType.ANALYSIS_PROBLEM);
		...
		if (resourcePostProcessorProvider != null) {
			if (resourcePostProcessorProvider instanceof edap.resource.edap.IEdapResourcePostProcessorProvider) {
				runPostProcessor(((edap.resource.edap.IEdapResourcePostProcessorProvider) resourcePostProcessorProvider).getResourcePostProcessor());
	...
	protected void runPostProcessor(edap.resource.edap.IEdapResourcePostProcessor postProcessor) {
		try {
	...
	public void addError(String message, org.eclipse.emf.ecore.EObject cause) {
		addError(message, edap.resource.edap.EdapEProblemType.UNKNOWN, cause);
	}
	
	public void addError(String message, edap.resource.edap.EdapEProblemType type, org.eclipse.emf.ecore.EObject cause) {
		addProblem(new edap.resource.edap.mopp.EdapProblem(message, type, edap.resource.edap.EdapEProblemSeverity.ERROR), cause);
	}
	
	@Deprecated	
	public void addWarning(String message, org.eclipse.emf.ecore.EObject cause) {
		addWarning(message, edap.resource.edap.EdapEProblemType.UNKNOWN, cause);
	}
	
	public void addWarning(String message, edap.resource.edap.EdapEProblemType type, org.eclipse.emf.ecore.EObject cause) {
		addProblem(new edap.resource.edap.mopp.EdapProblem(message, type, edap.resource.edap.EdapEProblemSeverity.WARNING), cause);
	}
	
	private java.util.List<org.eclipse.emf.ecore.resource.Resource.Diagnostic> getDiagnostics(edap.resource.edap.EdapEProblemSeverity severity) {
		if (severity == edap.resource.edap.EdapEProblemSeverity.ERROR) {
			return getErrors();
		} else {
			return getWarnings();
		}
	...
	protected void clearState() {
		...
		getWarnings().clear();
		if (isMarkerCreationEnabled()) {
			edap.resource.edap.mopp.EdapMarkerHelper.unmark(this, edap.resource.edap.EdapEProblemType.UNKNOWN);
			edap.resource.edap.mopp.EdapMarkerHelper.unmark(this, edap.resource.edap.EdapEProblemType.SYNTAX_ERROR);
			edap.resource.edap.mopp.EdapMarkerHelper.unmark(this, edap.resource.edap.EdapEProblemType.UNRESOLVED_REFERENCE);
		}
		proxyCounter = 0;
		resolverSwitch = null;
	}
