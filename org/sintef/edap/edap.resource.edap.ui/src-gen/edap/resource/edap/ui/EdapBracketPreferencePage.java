 ...
 * </ul>
 */
public class EdapBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new edap.resource.edap.mopp.EdapMetaInformation().getSyntaxName();
	
	private edap.resource.edap.ui.EdapBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public EdapBracketPreferencePage() {
		super();
		
		edap.resource.edap.IEdapMetaInformation metaInformation = new edap.resource.edap.mopp.EdapMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		...
		bracketsTmp = new edap.resource.edap.ui.EdapBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					edap.resource.edap.IEdapBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					edap.resource.edap.IEdapBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(		edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(		language + edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof edap.resource.edap.ui.EdapEditor) {
			((edap.resource.edap.ui.EdapEditor) editor).invalidateTextRepresentation();
		}
	}
