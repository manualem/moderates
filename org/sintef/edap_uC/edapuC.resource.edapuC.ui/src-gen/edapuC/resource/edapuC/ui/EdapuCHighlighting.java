 ...
 */
public class EdapuCHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private final static edapuC.resource.edapuC.ui.EdapuCPositionHelper positionHelper = new edapuC.resource.edapuC.ui.EdapuCPositionHelper();
	private boolean isHighlightBrackets = true;
	private edapuC.resource.edapuC.ui.EdapuCTokenScanner scanner;
	...
	private edapuC.resource.edapuC.ui.EdapuCOccurrence occurrence;
	private edapuC.resource.edapuC.ui.EdapuCBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, edapuC.resource.edapuC.IEdapuCBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public EdapuCHighlighting(edapuC.resource.edapuC.IEdapuCTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, edapuC.resource.edapuC.ui.EdapuCColorManager colorManager, edapuC.resource.edapuC.ui.EdapuCEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = edapuC.resource.edapuC.ui.EdapuCUIPlugin.getDefault().getPreferenceStore();
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new edapuC.resource.edapuC.ui.EdapuCTokenScanner(textResource, colorManager);
		occurrence = new edapuC.resource.edapuC.ui.EdapuCOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new edapuC.resource.edapuC.ui.EdapuCBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(edapuC.resource.edapuC.ui.EdapuCPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, edapuC.resource.edapuC.ui.EdapuCPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(edapuC.resource.edapuC.ui.EdapuCEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, edapuC.resource.edapuC.ui.EdapuCPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, edapuC.resource.edapuC.ui.EdapuCPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(edapuC.resource.edapuC.ui.EdapuCPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new edapuC.resource.edapuC.ui.EdapuCEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
	...
	private void handleContentOutlineSelection(org.eclipse.jface.viewers.ISelection selection) {
		if (!selection.isEmpty()) {
			...
			if (selectedElement instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (resource instanceof edapuC.resource.edapuC.IEdapuCTextResource) {
