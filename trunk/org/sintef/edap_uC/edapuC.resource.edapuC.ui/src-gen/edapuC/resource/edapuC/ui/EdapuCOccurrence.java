 ...
 */
public class EdapuCOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "edapuC.resource.edapuC.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "edapuC.resource.edapuC.ui.occurences.declaration";
	...
	private edapuC.resource.edapuC.ui.EdapuCTokenScanner tokenScanner;
	...
	private edapuC.resource.edapuC.IEdapuCTextResource textResource;
	 ...
	 */
	public EdapuCOccurrence(edapuC.resource.edapuC.IEdapuCTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, edapuC.resource.edapuC.ui.EdapuCTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new edapuC.resource.edapuC.mopp.EdapuCMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(edapuC.resource.edapuC.ui.EdapuCBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, edapuC.resource.edapuC.ui.EdapuCPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, edapuC.resource.edapuC.ui.EdapuCPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, edapuC.resource.edapuC.ui.EdapuCPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == edapuC.resource.edapuC.ui.EdapuCPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(edapuC.resource.edapuC.ui.EdapuCOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(edapuC.resource.edapuC.ui.EdapuCOccurrence.DECLARATION_ANNOTATION_ID);
	}
