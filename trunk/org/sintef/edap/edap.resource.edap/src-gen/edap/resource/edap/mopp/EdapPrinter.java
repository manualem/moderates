...
public class EdapPrinter implements edap.resource.edap.IEdapTextPrinter {
	...
	protected void doPrint(org.eclipse.emf.ecore.EObject element, java.io.PrintWriter out, String globaltab) {
		...
		if (element instanceof edap.DictionaryNavigation) {
			print_edap_DictionaryNavigation((edap.DictionaryNavigation) element, globaltab, out);
			return;
		}
	...
	protected void addWarningToResource(final String errorMessage, org.eclipse.emf.ecore.EObject cause) {
		...
		resource.addProblem(new edap.resource.edap.mopp.EdapProblem(errorMessage, edap.resource.edap.EdapEProblemType.ERROR), cause);
	}
	...
	public void print_edap_Property_1(edap.Property element, String outertab, java.io.PrintWriter out, java.util.Map<String, Integer> printCountingMap){
		...
		if (count > 0) {
			...
			if (o != null) {
				edap.resource.edap.IEdapTokenResolver resolver = tokenResolverFactory.createTokenResolver("MULTIPLICITY");
				resolver.setOptions(getOptions());
	...
	public void print_edap_ComponentReference(edap.ComponentReference element, String outertab, java.io.PrintWriter out) {
		...
		temp = element.eGet(element.eClass().getEStructuralFeature(edap.EdapPackage.COMPONENT_REFERENCE__PROPERTY_NAVIGATIONS));
		...
		while (iterate) {
			...
			out1 = new java.io.PrintWriter(sWriter);
			printCountingMap1 = new java.util.LinkedHashMap<String, Integer>(printCountingMap);
			print_edap_ComponentReference_0(element, localtab, out1, printCountingMap1);
			if (printCountingMap.equals(printCountingMap1)) {
				iterate = false;
				out1.close();
			} else {
	...
	public void print_edap_ComponentReference_0(edap.ComponentReference element, String outertab, java.io.PrintWriter out, java.util.Map<String, Integer> printCountingMap){
		String localtab = outertab;
		int count;
		// DEFINITION PART BEGINS (Containment)
		count = printCountingMap.get("propertyNavigations");
		if (count > 0) {
			Object o = element.eGet(element.eClass().getEStructuralFeature(edap.EdapPackage.COMPONENT_REFERENCE__PROPERTY_NAVIGATIONS));
			java.util.List<?> list = (java.util.List<?>) o;
			int index = list.size() - count;
			if (index >= 0) {
				o = list.get(index);
	...
	public void print_edap_DictionaryNavigation(edap.DictionaryNavigation element, String outertab, java.io.PrintWriter out) {
		String localtab = outertab;
		// The printCountingMap contains a mapping from feature names to the number of
		// remaining elements that still need to be printed. The map is initialized with
		// the number of elements stored in each structural feature. For lists this is the
		...
		java.util.Map<String, Integer> printCountingMap = new java.util.LinkedHashMap<String, Integer>(2);
		Object temp;
		temp = element.eGet(element.eClass().getEStructuralFeature(edap.EdapPackage.DICTIONARY_NAVIGATION__PROPERTY));
		printCountingMap.put("property", temp == null ? 0 : 1);
		temp = element.eGet(element.eClass().getEStructuralFeature(edap.EdapPackage.DICTIONARY_NAVIGATION__INDEX));
		...
		count = printCountingMap.get("property");
		if (count > 0) {
			Object o = element.eGet(element.eClass().getEStructuralFeature(edap.EdapPackage.DICTIONARY_NAVIGATION__PROPERTY));
			if (o != null) {
				edap.resource.edap.IEdapTokenResolver resolver = tokenResolverFactory.createTokenResolver("TEXT");
				resolver.setOptions(getOptions());
				out.print(resolver.deResolve(getReferenceResolverSwitch() == null ? null : getReferenceResolverSwitch().getPropertyNavigationPropertyReferenceResolver().deResolve((edap.Property) o, element, (org.eclipse.emf.ecore.EReference) element.eClass().getEStructuralFeature(edap.EdapPackage.DICTIONARY_NAVIGATION__PROPERTY)), element.eClass().getEStructuralFeature(edap.EdapPackage.DICTIONARY_NAVIGATION__PROPERTY), element));
			}
			printCountingMap.put("property", count - 1);
		}
		// DEFINITION PART BEGINS (CsString)
		...
		if (count > 0) {
			Object o = element.eGet(element.eClass().getEStructuralFeature(edap.EdapPackage.DICTIONARY_NAVIGATION__INDEX));
			if (o != null) {
				doPrint((org.eclipse.emf.ecore.EObject) o, out, localtab);
			}
