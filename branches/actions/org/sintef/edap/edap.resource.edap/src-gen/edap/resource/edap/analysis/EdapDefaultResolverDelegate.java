...
public class EdapDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	private static class ReferenceCache implements edap.resource.edap.IEdapReferenceCache, org.eclipse.emf.common.notify.Adapter {
	 ...
	 */
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, edap.resource.edap.IEdapReferenceResolveResult<ReferenceType> result) {
		try {
			org.eclipse.emf.ecore.EObject root = container;
			if (!enableScoping) {
				root = edap.resource.edap.util.EdapEObjectUtil.findRootContainer(container);
			}
			while (root != null) {
	...
	private java.util.Set<org.eclipse.emf.ecore.EObject> findExternalReferences(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = edap.resource.edap.util.EdapEObjectUtil.findRootContainer(object);
	 ...
	 */
	protected boolean tryToResolveIdentifierInObjectTree(String identifier, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject root, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, edap.resource.edap.IEdapReferenceResolveResult<ReferenceType> result, boolean checkRootFirst) {
	...
	private boolean tryToResolveIdentifierAsURI(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, edap.resource.edap.IEdapReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	private boolean checkElement(org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EReference reference, int position, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, edap.resource.edap.IEdapReferenceResolveResult<ReferenceType> result) {
		...
		if (exactMatch == null) {
			...
			if (similarMatch != null) {
				...
				result.addQuickFix(new edap.resource.edap.mopp.EdapChangeReferenceQuickFix("Replace with " + similarMatch, "IMG_TOOL_FORWARD", container, reference, oldTarget, element));
			}
			return true;
		}
	...
	protected String produceDeResolveErrorMessage(org.eclipse.emf.ecore.EObject refObject, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, edap.resource.edap.IEdapTextResource resource) {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation o : element.eClass().getEAllOperations()) {
				if (o.getName().toLowerCase().endsWith(NAME_FEATURE) && o.getEParameters().size() == 0 ) {
					String result = (String) edap.resource.edap.util.EdapEObjectUtil.invokeOperation(element, o);
	...
	protected edap.resource.edap.IEdapReferenceCache getCache(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = edap.resource.edap.util.EdapEObjectUtil.findRootContainer(object);
	...
	private boolean isSimilar(String identifier, Object attributeValue) {
		if (attributeValue != null && attributeValue instanceof String) {
			String name = (String) attributeValue;
			if (edap.resource.edap.util.EdapStringUtil.computeLevenshteinDistance(identifier, name) <= MAX_DISTANCE) {
				return true;
			}
		}
		return false;
