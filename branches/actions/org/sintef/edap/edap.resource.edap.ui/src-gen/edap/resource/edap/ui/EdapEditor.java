...
/**
 * A text editor for 'edap' models.
 */
public class EdapEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider {
	
	private edap.resource.edap.ui.EdapHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private edap.resource.edap.ui.EdapCodeFoldingManager codeFoldingManager;
	private edap.resource.edap.ui.EdapBackgroundParsingStrategy bgParsingStrategy = new edap.resource.edap.ui.EdapBackgroundParsingStrategy();
	private java.util.Collection<edap.resource.edap.IEdapBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<edap.resource.edap.IEdapBackgroundParsingListener>();
	private edap.resource.edap.ui.EdapColorManager colorManager = new edap.resource.edap.ui.EdapColorManager();
	private edap.resource.edap.ui.EdapOutlinePage outlinePage;
	...
	private edap.resource.edap.ui.EdapPropertySheetPage propertySheetPage;
	...
	private edap.resource.edap.ui.IEdapBracketHandler bracketHandler;
	
	public EdapEditor() {
		super();
		setSourceViewerConfiguration(new edap.resource.edap.ui.EdapEditorConfiguration(this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), EdapEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								edap.resource.edap.IEdapTextResource currentResource = getResource();
				...
				edap.resource.edap.ui.EdapUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("edap.resource.edap.EditorContext");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new edap.resource.edap.ui.EdapHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new edap.resource.edap.ui.EdapCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		edap.resource.edap.mopp.EdapNature.activate(inputFile.getProject());
		...
		edap.resource.edap.IEdapTextResource loadedResource = (edap.resource.edap.IEdapTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				edap.resource.edap.IEdapTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof edap.resource.edap.IEdapTextResource) {
					setResource((edap.resource.edap.IEdapTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					edap.resource.edap.ui.EdapUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the EdapEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				edap.resource.edap.ui.EdapUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(edap.resource.edap.IEdapTextResource resource) {
		assert resource != null;
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new edap.resource.edap.ui.EdapOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new edap.resource.edap.ui.EdapPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			edap.resource.edap.IEdapTextResource textResource = (edap.resource.edap.IEdapTextResource) element.eResource();
			...
			edap.resource.edap.IEdapTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				edap.resource.edap.IEdapTextToken token = lexer.getNextToken();
			...
			edap.resource.edap.ui.EdapUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	public void addBackgroundParsingListener(edap.resource.edap.IEdapBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (edap.resource.edap.IEdapBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public edap.resource.edap.ui.IEdapBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(edap.resource.edap.ui.IEdapBracketHandler bracketHandler) {
