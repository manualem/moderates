...
public class EdapBracketSet {
	...
	private java.util.ArrayList<edap.resource.edap.IEdapBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements edap.resource.edap.IEdapBracketPair {
	 ...
	 */
	private class ClosingListener implements edap.resource.edap.ui.IEdapBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				edap.resource.edap.IEdapBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public EdapBracketSet(edap.resource.edap.ui.EdapEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new edap.resource.edap.mopp.EdapMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<edap.resource.edap.IEdapBracketPair>();
	 ...
	 */
	public edap.resource.edap.IEdapBracketPair getBracketPair(String opening, String closing) {
		for (edap.resource.edap.IEdapBracketPair bracketPair : bracketPairs) {
	...
	public edap.resource.edap.IEdapBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(edap.resource.edap.IEdapBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + edap.resource.edap.ui.EdapPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public edap.resource.edap.IEdapBracketPair remove(String opening, String closing) {
		for (edap.resource.edap.IEdapBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<edap.resource.edap.IEdapBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, edap.resource.edap.ui.EdapPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, edap.resource.edap.ui.EdapPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
